...
void GltfSceneViewer::Setup()
{
    ...
    // Load GLTF scene
    {
        ...
        PPX_CHECKED_CALL(scene::GltfLoader::Create(GetAssetPath(GetExtraOptions().GetExtraOptionValueOrDefault("gltf-scene-file", defaultScene)), nullptr, &pLoaderRaw));
    ...
    // Pipeline args
    bool loadStandardMaterial = false;
    bool loadUnlitMaterial    = false;
    {
        ...
        frame.cmd->BeginRenderPass(&beginInfo);
        {
            ...
            for (uint32_t instanceIdx = 0; instanceIdx < numMeshNodes; ++instanceIdx) {
                ...
                for (auto& batch : batches) {
                    ...
                    // Set pipeline
                    auto                    pipelineIterator = mMaterialPipelineMap.find(material);
                    grfx::GraphicsPipeline* pipeline         = pipelineIterator == mMaterialPipelineMap.end() ? mErrorMaterialPipeline.Get() : pipelineIterator->second;
